    ...
    def put(self,request,*args,**kwargs):
        ...
        if checkoutSerializer.is_valid():
            ...
            if len(deleted_details)>0:
                open_order.refresh_from_db()
                serializer=OrderSerializer(open_order)
                raise CustomAPIException("The book(s) you have added to your cart is not available anymore! {}".format(*deleted_details),status=404,data=serializer.data)
            try:
                ...
                with transaction.atomic():
                    ...
                    open_order.save()
                    payment=createMolliePayment(str(open_order.cost),open_order.id,checkoutSerializer.validated_data.get('redirectUrl'))
            except:
                open_order.refresh_from_db()
                raise CustomAPIException("Checkout can not process, please try again later.", 500, data=open_order)

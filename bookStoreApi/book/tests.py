...
class AddBookTests(APITestCase):
     def setUp(self):
        ...
        self.book_data={"author":"Orhan Pamuk",
        ...
        with open('media/book_images/test_image.jpg', 'rb') as img:
            uploaded_image = SimpleUploadedFile("test_image.jpg", img.read(), content_type="image/jpeg")
     ...
     def test_add_book_success(self):
       ...
       Add book in success with 1 image.
        """
         ...
         response=self.client.post(self.add_book_url,self.book_data,format="multipart")
         ...
         self.assertEqual(self.book_data["publishing_house"], response.data["publishing_house"])
     ...
     def test_add_book_fail_missing_author(self):
       ...
       Add book in fail (missing author).
        """
         ...
         self.client.credentials(HTTP_AUTHORIZATION='Token ' + self.token.key)
         response=self.client.post(self.add_book_url,self.book_data,format="multipart")
         self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST,f"{response}")
     ...
     def test_add_book_fail_existed_book(self):
       ...
       Add book in fail (already exist ean).
        """
         ...
         response2=self.client.post(self.add_book_url,self.book_data,format="multipart")
     ...
     def test_add_book_fail_unauthorized(self):
       ...
       Add book in fail unauthorized.
        """
         response=self.client.post(self.add_book_url,self.book_data,format="multipart")
         self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED,f"{response}")
